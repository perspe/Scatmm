...
def _tauc_lorentz_peak(
    e_array: npt.NDArray[np.floating], eg: float, e0: float, a: float, c: float
) -> DispType:
    ...
    zeta4: npt.NDArray[np.floating] = np.power(e2 - gamma ** 2, 2) + alpha ** 2 * c2 / 4
    ...
    er -= (
        (a * atan)
        / (np.pi * zeta4 * e0)
        * (np.pi - np.arctan((2 * eg + alpha) / c) + np.arctan((alpha - 2 * eg) / c))
    )
    er += (4 * a * e0 * eg * (e2 - gamma ** 2) / (np.pi * zeta4 * alpha)) * (
        np.arctan((alpha + 2 * eg) / c) + np.arctan((alpha - 2 * eg) / c)
    )
    er -= (a * e0 * c * (e2 + eg2) / (np.pi * zeta4 * e_array)) * np.log(
        np.abs(e_array - eg) / (e_array + eg)
    )
    er += (2 * a * e0 * c * eg / (np.pi * zeta4)) * np.log(
        np.abs(e_array - eg) * (e_array + eg) / (np.sqrt((e02 - eg2) ** 2 + eg2 * c2))
    )
    return er, ei


def tauc_lorentz_1(
    e_array: npt.NDArray[np.floating],
    einf: float,
    eg: float,
    e0: float,
    a: float,
...
def sellmeier_abs(
    e_array: npt.NDArray[np.floating], a: float, b: float, c: float, d: float, e: float
) -> DispType:
...
def _new_amorphous_peak(
    e_array: npt.NDArray[np.floating], fj: float, gammaj: float, wj: float, wg: float
) -> DispType:
    k = np.zeros_like(e_array, dtype=np.float64)
    n = np.zeros_like(e_array, dtype=np.float64)
    w_mask = e_array > wg
    k[w_mask] = (
        fj * (e_array[w_mask] - wg) ** 2 / ((e_array[w_mask] - wj) ** 2 + gammaj ** 2)
    )
